<template>
  <div class="mx-datepicker"
       :style="{'width': width + 'px','min-width':range ? (type === 'datetime' ? '320px' : '190px') : '140px'}"
    ...
    <span class="mx-input-icon">
      <i data-v-4948f6ac="" data-v-8db4ec4a="" class="fa fa-calendar"></i>
    </span>
    <div class="mx-datepicker-popup animated"
         :class="{'range':range}"
         :style="position"
         ref="calendar"
         v-show="showPopup">
      ...
      <div v-else style="overflow:hidden">
        <div class="mx-datepicker-top" v-if="ranges.length">
          ...
          <span v-if="isValidRange(currentValue)" @click="clear">Clear</span>
        </div>
      ...
      <div class="mx-datepicker-footer" v-if="confirm">
        <a class="button is-gradient is-small" @click="confirmDate"> {{ confirmText }}</a>
        <a class="button is-gradient is-small" @click="closePopup">Close</a>
      </div>
    </div>
  </div>
</template>
  ...
  methods: {
    ...
    togglePopup () {
      if (this.showPopup) {
        ...
        let currentInput = this.$refs.input.value.split(' ~ ')
        if (currentInput.length === 2) {
          this.$emit('input', [new Date(currentInput[0]), new Date(currentInput[1])])
        } else {
          this.$emit('input', '')
        }
    ...
    clear () {
      this.currentValue = [undefined, undefined]
    },
...
<style lang="scss">
  .mx-datepicker {
    ...
    color: #363636;
    font: 0.8rem BlinkMacSystemFont, -apple-system, "Segoe UI", "Roboto", "Oxygen", "Ubuntu", "Cantarell", 
  ...
  .mx-input:focus ~ .mx-input-icon {
    color: #b5b5b5;
  }
  .mx-input:hover ~ .mx-input-icon {
    color: #b5b5b5;
  }
  .mx-input-icon {
    color: #dbdbdb;
